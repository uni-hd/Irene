...
hi=data.frame(hi[,c('gene','enh','dPC1')],d=rowMeans(hi[,c('start','end')])-rowMeans(hi[,c('V2','V3')]))
hi=hi[order(abs(hi$d)),]
hi=hi[match(unique(hi$gene),hi$gene),]
j=match(names(promrank[[i]]),hi$gene)
j[is.na(j)]=max(j[!is.na(j)])+1
d=abs(hi$dPC1)
d[length(d)]=0
d=d[j]
names(d)=names(promrank[[i]])
d=d+promrank[[i]]
gsub("_\\d+", "", names(sort(d,decreasing=TRUE)))
})
#rewired
...
au=dframe(do.call(rbind,lapply(v,function(i)c(get.auc(get.rank(markers[[i]], get.generank(rank[[i]]))),get.auc(get.rank(markers[[i]], prom[[i]])),get.auc(get.rank(markers[[i]], nearest[[i]]))))),col.names=c('Irene','Promoter','Nearest'),m.colnames=c('Type','Method','value'),melt=T)
...
i=2
th=1:4000
genes=getKEGGnodeNames('~/tmp/kegg/hsa05214.xml')
enh1=lapply(rank,function(d)names(get.generankscore(d))[th])
prom1=lapply(promrank,function(d)names(get.generankscore(d))[th])
lapply(1:7,function(i) write.table(enh1[[i]],file=paste0('~/tmp/i/',i,'.rnk'),quote=F,sep="\t",col.names=F))
link=getKEGGlink('hsa05214',getKEGGnodeId(prom1[[i]],genes),getKEGGnodeId(enh1[[i]],genes))
g=igraph.from.graphNEL(parseKGML2Graph('~/tmp/kegg/hsa05200.xml'))
nodes=getKEGGnodeNames('~/tmp/kegg/hsa05200.xml')
nodes$V1=paste0('hsa:',nodes$V1)
lapply(1:7,function(i) median(degree(g,mode='out')[getKEGGnodeId(prom1[[i]],nodes)]))
lapply(1:7,function(i) median(degree(g,mode='out')[getKEGGnodeId(enh1[[i]],nodes)]))
