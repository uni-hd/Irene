...
## {.tabset .tabset-fade}
Use the tabs to switch to the corresponding test cases and results.
...
```{r, eval=FALSE}
m=matrix(NA,6,6)
for(i in 1:6){for(j in 1:6){m[i,j]=cor(dat$Dobs[,i],dat$Dobs[,j])}}
...
```{r pathway}
th=1:4000
fn=read.table('res/keggpath.txt')[1,]
...
bsselect(sn, type = "iframe", selected = 'PATHWAYS_IN_CANCER', live_search = TRUE, show_tick = TRUE)
```
